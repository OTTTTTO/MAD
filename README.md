# MAD (Multi-Agent Discussion) v3.7.0
...
[![Version](https://img.shields.io/badge/version-3.7.0-green)](https://github.com/OTTTTTO/MAD)
...
**概念统一**
- ❌ v3.6.x: Discussion + ProjectGroup 双系统混乱
- ✅ v3.7.0: 统一为 Discussion（讨论组）概念
- ✅ 所有功能集中在单一系统中
...
Multi-Agent Discussion v3.7.0 是一个 OpenClaw Skill，让多个专业 Agent 在智能讨论组中协同工作。Agent 之间可以互相 @、回应观点、形成共识，最终产生比单个 Agent 更全面的解决方案。
...
// 克隆并指定新名称
const renamed = await v3.cloneProject('group-xxx', '新项目名称');
// 返回：新项目，使用指定的名称
...
### 场景 11：项目备注（v3.6.0 新增）

```javascript
// 设置项目备注
await v3.setProjectNotes('group-xxx', '这是一个重要的项目，需要优先处理');

// 追加项目备注（自动添加时间戳）
await v3.appendProjectNotes('group-xxx', '已完成第一阶段开发');

// 获取项目备注
const notes = await v3.getProjectNotes('group-xxx');
