...
async function runTests() {
  ...
  console.log(`\x1b[32m✓ 找到 ${similar1 ? similar1.length : 0} 个与讨论 1 相似的讨论\x1b[0m`);
  if (similar1 && similar1.length > 0) {
    similar1.forEach(item => {
      console.log(`  - ${item.topic} (${Math.round(item.similarity * 100)}%)`);
      if (item.commonKeywords && item.commonKeywords.length > 0) {
  ...
  console.log(`\x1b[32m✓ 找到 ${similar2 ? similar2.length : 0} 个与讨论 3 相似的讨论\x1b[0m`);
